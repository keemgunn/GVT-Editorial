...
const routeSettings = import.meta.glob('../../src/pages/**/routeSetting.ts', { eager: true, import: 'default' });

export function createRouterRecordArray(): Array<RouteRecordRaw> {

  // Get array of PageComponent path list
  const pageComponentPaths = searchModules(import.meta.glob('../../src/pages/**/PageComponent.vue'));
  ...
  pageComponentPaths.forEach((path) => {
    ...
    const routeSetting = routeSettings[`../../src/pages/${componentName}/routeSetting.ts`] as PageRouteSetting;
  
    const record = {
      name: componentName,
      path: routeSetting.uri,
      alias: routeSetting.alias || [],
      component: () => import(`../../src/pages/${componentName}/PageComponent.vue`),
